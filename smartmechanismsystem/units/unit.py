...
class Unit():
    ...
    def conversion_factor_from(self, other: "Unit") -> float:
        ...
        """
        if(self.TYPE != other.TYPE):
            raise UnitException("Cannot convert between different types of units.", "Cannot convert from " + str(type(self)) + " (" + self.name + ") to " + str(type(other)) + " (" + other.name + ").", "conversion_factor_from")
